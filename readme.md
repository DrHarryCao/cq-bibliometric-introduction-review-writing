...
python <PIPELINE> theory-library init
python <PIPELINE> theory-library ingest THEORY_FILE_OR_FOLDER [--recursive]
python <PIPELINE> theory-library verify --phase prepare|compile|validate
python <PIPELINE> theory-library status
python <PIPELINE> theory-library search --query TEXT
python <PIPELINE> theory-library export --output theory-library.zip
python <PIPELINE> theory-library import --bundle theory-library.zip
...
单文件用户说明书内嵌两个可展开的完整静态成果，用于展示最终交付物的篇章组织、学术排版和引文呈现：

- **SSCI漏斗型绪论（投稿正文版）**：完整展示11个连续正文自然段和146条参考文献，适合观察现实背景、知识基线、解释缺口、研究目标与贡献之间的漏斗衔接。
- **系统综述（证据提示版）**：完整展示51个正文段落、13个标题和177条参考文献，适合观察概念边界、机制综合、研究分歧、方法评估、Research Gap与研究议程如何形成解释性叙事。

两个示例均为**未验证草稿预览**，仅用于说明输出结构和写作组织，不代表其事实、理论、引文或APA细节已经完成最终审计。不得把示例中的论断、理论或参考文献迁移到无关研究任务。

完整正文不在README中重复，以避免仓库首页过长。请打开[单文件用户说明书](<CQ-Bibliometric-Introduction-Review-Writing- UserManual.html#demos>)并展开相应示例。
...
`status` reports `requires_user_input`, the current checkpoint, accepted replies, `next_command`, and whether the workflow can continue automatically. Technical recovery, evidence expansion, reference synchronization, validation, and export continue without returning control unless a genuine user decision is required.
...
The following `<PIPELINE>` denotes `cq-bibliometric-introduction-review-writing/scripts/review_pipeline.py` in the extracted release:

```text
python <PIPELINE> doctor --json
python <PIPELINE> wizard --task TASK --title "RESEARCH TOPIC" --mode api-search|strategy-only
python <PIPELINE> validate-plan --task TASK
python <PIPELINE> search-strategy --task TASK --mode api-search|strategy-only
python <PIPELINE> search --task TASK --confirm
python <PIPELINE> ingest --task TASK FILE_OR_FOLDER...
...
Run `status` before resuming. Prefer host conversation for ordinary work; direct CLI use is mainly for diagnostics, recovery, and advanced control.
...
### 17. Complete deliverable examples

The self-contained user manual embeds two expandable, complete static deliverables that demonstrate final document organization, academic layout, and citation presentation:

- **SSCI funnel-shaped introduction (publication version):** 11 continuous body paragraphs and 146 references, illustrating the progression from real-world context and knowledge baseline to explanatory gap, research objectives, and contribution.
- **Systematic review (evidence-aware version):** 51 body paragraphs, 13 headings, and 177 references, illustrating how construct boundaries, mechanism synthesis, disagreement, methodological appraisal, research gaps, and a research agenda form an explanatory narrative.

Both are **unvalidated draft previews**. They demonstrate output structure and writing organization only; their facts, theories, citations, and APA details are not represented as finally audited. Never transfer claims, theories, or references from these examples into an unrelated research task.

The complete manuscripts are not duplicated in the README. Open the [single-file user manual](<CQ-Bibliometric-Introduction-Review-Writing- UserManual.html#demos>) and expand either example.
