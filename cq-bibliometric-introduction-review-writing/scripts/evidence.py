...
from common import file_sha256, load_jsonl, normalize_title, read_csv_optional, read_json, write_json, write_jsonl
...
def build_evidence(root: Path) -> dict[str, Any]:
    ...
    coverage["corpus_records"] = len(records)
    ...
    for record in sorted(records, key=lambda r: (str((r.get("authors") or [""])[0]), r.get("year") or 0, r.get("title") or "")):
        ...
        registry.append({"record_id": record["record_id"], "apa": apa, "doi": (record.get("ids") or {}).get("doi", ""), "csl": csl, "metadata_status": "complete" if not missing_bib else "incomplete", "missing_fields": missing_bib})
