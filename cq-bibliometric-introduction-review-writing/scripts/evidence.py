...
def clean_introduction_text(text: str) -> str:
    clean = re.sub(r"\s*\[(?:cite:)?\s*(?:C\d{4}|R[0-9a-f]{14}|SCTX-\d{3,})(?:\s*[,;，；]\s*(?:C\d{4}|R[0-9a-f]{14}|SCTX-\d{3,}))*\s*\]", "", text, flags=re.I)
...
def sync_references(root: Path, document: str = "review", draft_path: Path | None = None) -> dict[str, Any]:
    ...
    if document == "introduction":
        (root / "06_review/ssci_introduction.md").write_text(clean_introduction_text(path.read_text(encoding="utf-8")), encoding="utf-8")
