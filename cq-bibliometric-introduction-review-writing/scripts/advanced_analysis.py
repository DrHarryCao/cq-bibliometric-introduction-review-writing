...
TERM_VARIANTS = {"ecommerce": "e-commerce", "e commerce": "e-commerce", "livestreaming": "live-streaming", "live streaming": "live-streaming", "information overloads": "information overload", "purchase intentions": "purchase intention"}


def normalize_text(value: str) -> str:
    text = unicodedata.normalize("NFKC", str(value or "")).casefold()
    text = re.sub(r"[\u2010-\u2015]", "-", text)
    for source, target in TERM_VARIANTS.items(): text = re.sub(rf"\b{re.escape(source)}\b", target, text)
    return re.sub(r"\s+", " ", text).strip()


def language_tokens(value: str) -> list[str]:
    text = normalize_text(value); tokens: list[str] = []
    for chunk in re.findall(r"[a-z][a-z0-9-]{1,}|[\u4e00-\u9fff]+", text):
        if re.fullmatch(r"[\u4e00-\u9fff]+", chunk):
            try:
                import jieba
                jieba.setLogLevel(logging.ERROR)
                parts = [x.strip() for x in jieba.cut(chunk, cut_all=False) if x.strip()]
            except Exception:
                parts = [chunk[i:i + 2] for i in range(max(1, len(chunk) - 1))]
            tokens.extend(parts)
        else: tokens.append(chunk)
    return [x for x in tokens if x not in STOP and len(x) > 1]
...
def preprocessing_audit(records: list[dict[str, Any]]) -> pd.DataFrame:
    rows = []
    for record in records:
        raw = f"{record.get('title', '')} {record.get('abstract', '')}".strip(); normalized = normalize_text(raw); tokens = language_tokens(normalized)
        rows.append({"record_id": record.get("record_id"), "detected_cjk": bool(re.search(r"[\u4e00-\u9fff]", raw)), "raw_characters": len(raw), "normalized_characters": len(normalized), "tokens": len(tokens), "sample_tokens": "; ".join(tokens[:30])})
...
def vectorize(records: list[dict[str, Any]]):
    ...
    vectorizer = TfidfVectorizer(
        ...
        tokenizer=language_tokens, token_pattern=None, lowercase=False, sublinear_tf=True,
    )
    try:
...
def _topic_coherence(component: np.ndarray, binary: np.ndarray, top_n: int = 10) -> float:
    ...
    for a, b in combinations(ids, 2):
        both = float(np.logical_and(binary[:, a], binary[:, b]).sum()); pa = float(binary[:, a].mean()); pb = float(binary[:, b].mean()); pab = both / max(binary.shape[0], 1)
        if pab > 0 and pa > 0 and pb > 0:
            pmi = math.log(pab / (pa * pb)); scores.append(pmi / max(-math.log(pab), 1e-12))
    return float(np.mean(scores)) if scores else 0.0


def _bootstrap_topic_stability(matrix, reference: np.ndarray, k: int, runs: int, seed: int = 173) -> tuple[float, float]:
    if runs <= 0 or matrix.shape[0] < max(10, k * 2): return 0.0, 0.0
    from scipy.optimize import linear_sum_assignment
    rng = np.random.default_rng(seed); scores = []
    strata = np.asarray((matrix @ reference.T).argmax(axis=1)).ravel()
    for index in range(runs):
        selected_parts = []
        for topic in range(k):
            members = np.flatnonzero(strata == topic)
            if len(members): selected_parts.append(rng.choice(members, size=max(2, int(len(members) * .8)), replace=True))
        selected = np.concatenate(selected_parts) if selected_parts else rng.choice(matrix.shape[0], size=max(k * 2, int(matrix.shape[0] * .8)), replace=True)
        model = NMF(n_components=k, init="nndsvdar", random_state=seed + index, max_iter=800).fit(matrix[selected])
        left = normalize(reference); right = normalize(model.components_); similarity = left @ right.T
        row, col = linear_sum_assignment(-similarity); scores.append(float(np.mean(similarity[row, col])))
    return float(np.mean(scores)), float(min(scores))


def nmf_topics(records: list[dict[str, Any]], min_k: int = 4, max_k: int = 8, requested_k: int = 0, bootstrap_runs: int = 5):
    ...
    empty_diag = pd.DataFrame(columns=["k", "reconstruction_error", "coherence", "stability", "bootstrap_stability", "bootstrap_min", "preprocessing_stability", "topic_diversity", "exclusivity", "iterations", "converged", "min_topic_size", "score"])
    ...
    try:
        alternative_matrix = TfidfVectorizer(analyzer="char_wb", ngram_range=(3, 5), min_df=2 if len(valid) >= 20 else 1, max_df=.97, max_features=4000, sublinear_tf=True).fit_transform([modeling_text(records[i]) for i in valid])
    except ValueError:
        alternative_matrix = None
    ...
    for k in candidates:
        ...
        bootstrap_stability, bootstrap_min = _bootstrap_topic_stability(matrix, model.components_, k, bootstrap_runs)
        if alternative_matrix is not None and k <= min(alternative_matrix.shape):
            alternative_labels_preprocessing = NMF(n_components=k, init="nndsvda", random_state=42, max_iter=800).fit_transform(alternative_matrix).argmax(axis=1)
            preprocessing_stability = _adjusted_rand(labels, alternative_labels_preprocessing)
        else:
            preprocessing_stability = 0.0
        top_sets = [set(c.argsort()[::-1][:10]) for c in model.components_]; diversity = len(set().union(*top_sets)) / max(k * 10, 1)
        normalized_components = model.components_ / np.maximum(model.components_.sum(axis=0, keepdims=True), 1e-12)
        exclusivity = float(np.mean([np.mean(normalized_components[i, list(top_sets[i])]) for i in range(k)]))
        ...
        score = coherence * 0.20 + stability * 0.18 + bootstrap_stability * 0.22 + preprocessing_stability * .12 + diversity * 0.10 + min(balance, 1.0) * 0.18 - recon * 0.05
        diagnostics.append({"k": k, "reconstruction_error": round(recon, 6), "coherence": round(coherence, 6), "stability": round(stability, 6), "bootstrap_stability": round(bootstrap_stability, 6), "bootstrap_min": round(bootstrap_min, 6), "preprocessing_stability": round(preprocessing_stability, 6), "topic_diversity": round(diversity, 6), "exclusivity": round(exclusivity, 6), "iterations": int(model.n_iter_), "converged": bool(model.n_iter_ < model.max_iter), "min_topic_size": min_size, "score": round(score, 6)})
    ...
    meta = {"status": "ok", "method": "language-aware-tfidf-nmf", "selected_k": selected, "documents": len(valid), "features": int(matrix.shape[1]), "selection": "c-npmi+multi-seed+bootstrap+preprocessing-sensitivity+diversity+balance+reconstruction", "random_seeds": [42, 73, 101, 151], "bootstrap_runs": bootstrap_runs, "soft_membership": True, "selected_converged": bool(chosen_diagnostic["converged"]), "selected_stability": float(chosen_diagnostic["stability"]), "selected_bootstrap_stability": float(chosen_diagnostic["bootstrap_stability"]), "selected_preprocessing_stability": float(chosen_diagnostic["preprocessing_stability"]), "strong_bibliometric_claims_allowed": bool(chosen_diagnostic["converged"] and chosen_diagnostic["stability"] >= 0.75 and chosen_diagnostic["bootstrap_stability"] >= .70 and chosen_diagnostic["preprocessing_stability"] >= .50)}
...
def extract_phrases(records: list[dict[str, Any]]) -> tuple[dict[str, set[str]], dict[str, str]]:
    ...
    vectorizer = TfidfVectorizer(ngram_range=(2, 4), min_df=2 if len(records) >= 20 else 1, max_features=5000, tokenizer=language_tokens, token_pattern=None, lowercase=False)
...
def extract_context_keywords(records: list[dict[str, Any]]) -> dict[str, set[str]]:
    texts = [modeling_text(r) for r in records]
    if not any(texts): return {}
    vectorizer = TfidfVectorizer(ngram_range=(1, 2), min_df=2 if len(records) >= 20 else 1, max_features=3000, tokenizer=language_tokens, token_pattern=None, lowercase=False)
...
def burst_tables(records: list[dict[str, Any]], keyword_getter, assignments: pd.DataFrame, min_docs: int = 3):
    ...
    totals = Counter(int(r["year"]) for r in records if r.get("year")); rec_topic = dict(zip(assignments.get("record_id", []), assignments.get("topic_id", []))); record_year = {r["record_id"]: int(r["year"]) for r in records if r.get("year")}
    ...
    def build(kind: str, getter):
        ...
        for term, record_ids in docs.items():
            ...
            for start, end, strength, peak in _burst_intervals(counts, [totals[y] for y in years], years):
                supporting = sorted(rid for rid in record_ids if start <= record_year.get(rid, -1) <= end)
                topics = sorted({int(rec_topic[rid]) for rid in supporting if rid in rec_topic})
                rows.append({"type": kind, "term": term, "start_year": start, "end_year": end, "duration": end-start+1, "strength": strength, "peak_year": peak, "documents": len(record_ids), "uncertainty": "moderate" if len(record_ids) >= 10 and end > start else "high", "null_baseline": "constant-rate-binomial-emission", "supporting_documents": "; ".join(supporting[:20]), "topic_ids": "; ".join(map(str, topics)), "representative_context": examples.get(term, "")})
    ...
    for r in records:
        ...
        for start, end, strength, peak in _burst_intervals(values, baseline, cyears):
            citation_rows.append({"record_id": r["record_id"], "title": r.get("title", ""), "start_year": start, "end_year": end, "duration": end-start+1, "strength": strength, "peak_year": peak, "uncertainty": "moderate" if end > start and sum(values) >= 10 else "high", "null_baseline": "constant-rate-binomial-emission"})
    ...
    return keyword_bursts, phrase_bursts, citation_bursts, {"status": "ok", "method": "kleinberg-two-state", "null_model": "constant-rate binomial emission with transition penalty", "uncertainty_levels": ["moderate", "high"], "years": years, "citation_history_records": sum(bool(r.get("citation_counts_by_year")) for r in records)}
