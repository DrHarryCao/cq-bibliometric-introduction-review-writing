...
def _keyring_set(service: str, value: str) -> None:
    try:
        ...
        raise RuntimeError("此系统需要 keyring 才能安全保存凭据：pip install keyring") from exc
...
def credential_status() -> dict[str, Any]:
    ...
    return {"configured": bool(items["OPENALEX_API_KEY"]["configured"]), "credentials": items, "secrets_exposed": False}
...
def test_credentials(timeout: int = 15) -> dict[str, Any]:
    ...
    request = urllib.request.Request(url, headers={"User-Agent": "cq-bibliometric-introduction-review-writing/3.0"})
    try:
...
def prompt_value(prompt: str, title: str, hidden: bool = False) -> str | None:
...
def configure_dialog(include_semantic_scholar: bool = False) -> dict[str, Any]:
    key = prompt_value("请输入 OpenAlex API Key（将安全保存到系统钥匙串，不会写入项目或聊天记录）：", "配置 OpenAlex", hidden=True)
    if key is None: return {"configured": False, "cancelled": True, "stored": []}
    if len(key.strip()) < 8: raise RuntimeError("OpenAlex API Key 为空或过短。")
    store_credential("OPENALEX_API_KEY", key)
    stored = ["OPENALEX_API_KEY"]
    email = prompt_value("请输入用于 Unpaywall/Crossref 的联系邮箱（建议填写，可取消跳过）：", "配置学术数据源", hidden=False)
    if email and "@" in email:
        store_credential("UNPAYWALL_EMAIL", email); store_credential("CROSSREF_EMAIL", email); stored += ["UNPAYWALL_EMAIL", "CROSSREF_EMAIL"]
    if include_semantic_scholar:
        s2 = prompt_value("请输入可选的 Semantic Scholar API Key（可取消跳过）：", "配置 Semantic Scholar", hidden=True)
        if s2: store_credential("S2_API_KEY", s2); stored.append("S2_API_KEY")
    show_message("凭据已安全保存。现在可以继续检索。")
    return {"configured": True, "cancelled": False, "stored": stored}
