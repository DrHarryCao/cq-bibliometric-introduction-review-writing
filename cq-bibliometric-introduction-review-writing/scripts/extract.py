...
def extract_pdf(path: Path, ocr: bool = False) -> tuple[str, dict[str, Any]]:
    ...
    for page_no, page in enumerate(doc, 1):
        ...
        if ocr and len(text) < 120:
            ...
            if not binary: raise RuntimeError("已请求 OCR，但系统找不到 tesseract 可执行文件。")
            try:
                import pytesseract
                from PIL import Image
            except ImportError as exc: raise RuntimeError("OCR 需要 pytesseract 和 Pillow。") from exc
            pytesseract.pytesseract.tesseract_cmd = binary
            available = set(pytesseract.get_languages(config="")); preferred = [x for x in ("chi_sim", "eng") if x in available]
            if not available: raise RuntimeError("Tesseract可用，但未检测到任何语言包。")
            pix = page.get_pixmap(matrix=fitz.Matrix(2, 2), alpha=False)
            text = pytesseract.image_to_string(Image.frombytes("RGB", [pix.width, pix.height], pix.samples), lang="+".join(preferred) if preferred else sorted(available)[0]).strip()
            ocr_pages += 1
        chars += len(text)
...
def convert_doc(path: Path) -> Path:
    libreoffice = executable("libreoffice")
