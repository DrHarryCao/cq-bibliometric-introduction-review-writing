...
def prepare_gaps(root: Path) -> dict[str, Any]:
    ...
    if table.exists():
        for row in pd.read_csv(table).fillna("").to_dict("records"):
    ...
    if counter.exists():
        frame = pd.read_csv(counter).fillna("")
        if not frame.empty:
    ...
    report = {"phase": "prepare", "status": "prepared", "candidates": len(candidates), "ledger": str(ledger), "host_action": "complete gap_ledger.jsonl from content evidence"}
