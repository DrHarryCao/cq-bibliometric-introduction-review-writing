...
def knowledge_flow(citation_edges: pd.DataFrame, assignments: pd.DataFrame, records: list[dict[str, Any]], mode: str = "auto") -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    # Permutation null preserves topic sizes and citation topology while breaking
    # the association between documents and topics.
    ids = list(topic); labels = np.array([topic[rid] for rid in ids]); rng = np.random.default_rng(42); observed_pairs = [(row["source_topic"], row["target_topic"]) for _, row in flows.iterrows()]; null = {pair: [] for pair in observed_pairs}
    edge_pairs = [(row["source_id"], row["target_id"], float(row.get("weight", 1))) for _, row in citation_edges.iterrows() if row["source_id"] in topic and row["target_id"] in topic]
    for _ in range(100):
        shuffled = dict(zip(ids, rng.permutation(labels))); totals = Counter()
        for citing, cited, weight in edge_pairs: totals[(shuffled[cited], shuffled[citing])] += weight
        for pair in observed_pairs: null[pair].append(totals.get(pair, 0.0) / math.sqrt(max(sizes[pair[0]] * sizes[pair[1]], 1)))
    null_means, null_sds, null_z = [], [], []
    for _, row in flows.iterrows():
        values = null.get((row["source_topic"], row["target_topic"]), [0.0] * 100); mean, sd = float(np.mean(values)), float(np.std(values))
        null_means.append(mean); null_sds.append(sd); null_z.append((float(row["fractional_flow"]) - mean) / sd if sd > 0 else None)
    flows["null_mean"] = null_means; flows["null_sd"] = null_sds; flows["null_z"] = null_z
    flows["uncertainty"] = ["moderate" if eligible and z is not None and z >= 1.96 else "high" for eligible, z in zip(flows["writing_eligible"], flows["null_z"])]
    flows["writing_eligible"] = flows["writing_eligible"] & (flows["null_z"].fillna(-np.inf) >= 1.96)
    ready = edge_count >= 50 and bool(flows["writing_eligible"].any())
    return detail, flows, {"module": "knowledge_flow", "status": "ready" if ready else "exploratory", "reason": "flows pass independent-document and permutation-null gates" if ready else "no flow passes both independent-document and permutation-null gates", "local_edges": edge_count, "null_runs": 100, "uncertainty": "moderate" if ready else "high", "writing_allowed": ready}
