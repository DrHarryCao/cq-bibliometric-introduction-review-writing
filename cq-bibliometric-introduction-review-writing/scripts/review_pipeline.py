...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    update_manifest(root, "init", {"title": args.title, "skill_schema_version": 6})
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    elif status["review_html"] and status["introduction_html"] and status["validation_valid"]: next_step = "任务已完成；如正文或语料变更，重新运行 validate 与 export-deliverables"
...
def parser() -> argparse.ArgumentParser:
    ...
    brief = sub.add_parser("writing-brief"); brief.add_argument("--task", required=True); brief.add_argument("--document", choices=["review", "introduction"], required=True); brief.add_argument("--gap", action="append"); brief.add_argument("--method-preference", action="append"); brief.add_argument("--method-constraint", action="append"); brief.add_argument("--skip", action="store_true"); brief.add_argument("--no-model-package", action="store_true"); brief.set_defaults(func=cmd_writing_brief)
