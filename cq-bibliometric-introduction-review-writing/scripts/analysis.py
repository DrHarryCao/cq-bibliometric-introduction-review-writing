...
from advanced_analysis import (
    ...
    nmf_topics, preprocessing_audit, topic_cluster_cross,
)
...
def network_sensitivity_rows(name: str, frame: pd.DataFrame, source: str, target: str, weight: str, counting: str) -> list[dict[str, Any]]:
    rows = []
    for threshold in (1, 2, 3):
        subset = frame[frame[weight] >= threshold] if not frame.empty else frame
        graph = nx.from_pandas_edgelist(subset, source, target) if not subset.empty else nx.Graph()
        observed = nx.average_clustering(graph) if graph.number_of_nodes() > 1 else 0.0; null_values = []
        if 2 <= graph.number_of_nodes() <= 5000 and graph.number_of_edges() > 0:
            for seed in range(10):
                random_graph = nx.gnm_random_graph(graph.number_of_nodes(), graph.number_of_edges(), seed=1000 + seed)
                null_values.append(nx.average_clustering(random_graph))
        null_mean = float(np.mean(null_values)) if null_values else 0.0; null_sd = float(np.std(null_values)) if null_values else 0.0
        rows.append({"network": name, "minimum_weight": threshold, "nodes_retained": graph.number_of_nodes(), "edges_retained": graph.number_of_edges(), "density": round(nx.density(graph), 6) if graph.number_of_nodes() > 1 else 0.0, "observed_clustering": round(observed, 6), "null_clustering_mean": round(null_mean, 6), "null_clustering_z": round((observed - null_mean) / null_sd, 6) if null_sd > 0 else None, "null_runs": len(null_values), "counting": counting, "interpretation": "robustness-signal-only"})
    return rows
...
def nmf_access_gate(model: dict[str, Any], topics: pd.DataFrame, total_records: int, mode: str = "auto") -> dict[str, Any]:
    """Separate use as a claim from use as an explicitly exploratory outline."""
    claim_allowed = bool(model.get("strong_bibliometric_claims_allowed"))
    text_coverage = float(model.get("documents") or 0) / max(total_records, 1)
    minimum_topic = int(topics["documents"].min()) if not topics.empty and "documents" in topics else 0
    structurally_usable = bool(model.get("selected_converged") and not topics.empty and text_coverage >= .60 and minimum_topic >= 3)
    structure_allowed = False if mode == "off" else (claim_allowed if mode == "strict" else structurally_usable)
    if claim_allowed:
        status, reason = "ready", "all convergence, seed, bootstrap and preprocessing-sensitivity gates passed"
    elif structure_allowed:
        status, reason = "exploratory-structure", "converged with adequate text/topic coverage, but stability gates failed; host semantic naming and explicit exploratory warning are required"
    else:
        status, reason = "unavailable-for-structure", "NMF is off, unconverged, empty, severely imbalanced, or has insufficient text coverage; organize prose from semantic evidence and the approved research question"
    return {"module": "nmf", "status": status, "claim_allowed": claim_allowed, "structure_allowed": structure_allowed, "writing_allowed": claim_allowed, "structure_mode": mode, "reason": reason, "selected_k": model.get("selected_k"), "text_coverage": round(text_coverage, 6), "minimum_topic_documents": minimum_topic, "semantic_renaming_required": bool(structure_allowed and not claim_allowed), "seed_stability": model.get("selected_stability"), "bootstrap_stability": model.get("selected_bootstrap_stability"), "preprocessing_stability": model.get("selected_preprocessing_stability")}


def analyze(root: Path, records: list[dict[str, Any]], topic_count: int = 0, nmf_min_k: int = 4, nmf_max_k: int = 8, cluster_min_k: int = 2, cluster_max_k: int = 10, burst_min_docs: int = 3, counting: str = "fractional", skip_kmeans: bool = False, strategic_mode: str = "auto", citation_age_mode: str = "auto", knowledge_flow_mode: str = "auto", language: str = "auto", bootstrap_runs: int = 5, nmf_structure_mode: str = "auto") -> dict[str, Any]:
    years = year_stats(records); terms = term_stats(records)
    topics, assignments, nmf_diagnostics, model = nmf_topics(records, nmf_min_k, nmf_max_k, topic_count, bootstrap_runs)
    preprocessing = preprocessing_audit(records)
    if skip_kmeans:
    ...
    sensitivity_rows = []
    for network, frame, source, target, weight in (("keyword_cooccurrence", keyword_edges, "term_a", "term_b", "cooccurrence"), ("author_collaboration", author_edges, "author_a", "author_b", "collaborations"), ("co_citation", cocitation_edges, "reference_a", "reference_b", "co_citations"), ("bibliographic_coupling", coupling_edges, "record_a", "record_b", "shared_references")):
        sensitivity_rows.extend(network_sensitivity_rows(network, frame, source, target, weight, counting))
    ...
    outputs["preprocessing_audit"] = preprocessing
    ...
    nmf_gate = nmf_access_gate(model, topics, len(records), nmf_structure_mode)
    parameters.update({"topic_structure_source": "NMF-only", "kmeans_role": "heterogeneity-diagnostic-only", "advanced_modules": {"nmf": nmf_gate, "kmeans": kmeans_gate, "strategic_map": strategic_status, "citation_age": citation_age_status, "knowledge_flow": flow_status}, "embedding_used": False})
