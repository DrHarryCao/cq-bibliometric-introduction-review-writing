...
4. 运行 `reconcile`，只对矛盾、术语错位、低置信及至少3项研究的高影响综合进行第二遍跨文献复核。
5. 对语义 claim 标记 `supported/partial/unsupported`，然后运行 `validate`。
6. `unsupported` 禁止定稿；`partial` 必须缩小措辞；强概括默认需至少3项独立研究。

缓存键由源文件哈希与提取协议版本共同构成；未变化文献不重复读取。
