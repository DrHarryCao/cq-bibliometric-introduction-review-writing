...
description: Map a research field and draft a traceable systematic review or funnel-shaped SSCI introduction from a title, idea, bibliographic exports, or local full texts, with optional API search or API-free WoS strategy generation, explanatory/evidential gap audits, dynamic research-design and method matching, theory-model packages, and auditable HTML/RIS delivery. Use for bibliometrics and Chinese academic writing. JWC💗XQ@Rednote drharry. Never bypass paywalls or fabricate unavailable evidence.
---
...
python "$SKILL_DIR/scripts/review_pipeline.py" build-gaps --task TASK --phase prepare|compile|validate
python "$SKILL_DIR/scripts/review_pipeline.py" recommend-design --task TASK --phase prepare|compile|validate
